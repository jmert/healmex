classdef healmex < matlab.mixin.CustomDisplay
  methods (Static)
    ...
    map = alm2map(alms, nside, order, lmax, mmax, rwghts)
    ...
    [almsG,almsC] = hpx_map2alm_polonly(nside, order, mapQ, mapU, lmax, mmax, rwghts, iter)
    ...
    [mapQ,mapU] = hpx_alm2map_polonly(lmax, mmax, almsG, almsC, nside, order, rwghts)
    ...
    amap = hpx_smooth_mask(nside, order, map, radius, rwghts)
	  ...
	  mcm = mask_bpwf(mask, lmax, lmax_mask, order, niter, rwghts, pureB)
  end
